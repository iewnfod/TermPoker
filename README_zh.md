...
### TermPoker：一款策略型终端扑克游戏

TermPoker 将中国经典纸牌游戏「掼蛋」的核心玩法带到你的终端中，提供完全无需图形界面的游戏体验。无论你使用的是 macOS、Linux、Windows（通过 WSL），还是通过 SSH 连接到远程服务器，TermPoker 都能直接在命令行中为你带来完整的打牌乐趣。

### 玩法策略与挑战

从核心来看，TermPoker 是一款融合策略、记忆与战术决策的游戏。你将与三名不同难度的 AI 对手同台竞技，需要谨慎管理手牌，智取对手。每一次决策都很重要——是打出一张单牌给对手施加压力，是将强力炸弹留到关键时刻，还是策略性地打出高点数牌以消除威胁？游戏奖励那些能够预测对手行为、记住已出牌张、并根据动态牌局调整策略的玩家。

三个难度级别确保 TermPoker 既能陪伴新手入门，也能满足老玩家的挑战欲。初学者可以在「记牌器」的辅助下，一边熟悉复杂的牌型规则与组合，一边与简单难度的 AI 对练；而经验丰富的玩家则可以挑战困难难度的 AI，后者采用了更精妙的策略与牌型识别逻辑。

### 沉浸式终端界面

尽管是基于终端的游戏，TermPoker 在用户体验上也毫不妥协。键盘交互界面让你可以通过方向键灵活移动光标，使用空格键选牌，按回车键出牌。我们将你的手牌以一行扑克牌图形的方式呈现，并支持实时的视觉反馈。

### 跨平台可访问性

TermPoker 完全使用 C++ 编写，不依赖任何图形界面库，因此轻量且具备出色的跨平台兼容性。无论是在本地游玩，还是通过 SSH 连接到远程服务器上的游戏进程，体验都完全一致。极低的资源占用意味着你可以将它与其他终端应用一同运行，而不会有任何额外负担。
